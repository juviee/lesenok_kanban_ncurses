...
public:
  inline void set_description(std::string new_description) { description = new_description; }
  inline void set_name(std::string new_name) { name = new_name; }
  inline void set_start_time(time_t new_start_time) { start_time = new_start_time; }
  inline void set_deadline(time_t new_deadline) { deadline = new_deadline; }
  inline void set_done() { status = true; }
  inline void set_undone() { status = false; }
  inline void set_id(int new_id) { id = new_id; }

  inline std::string get_name() { return name; }
  inline std::string get_description() { return description; }
  inline time_t get_start_time() { return start_time; }
  inline time_t get_deadline() { return deadline; }
  inline int get_id() { return id; }
  inline virtual std::pair<int, int> get_status() { return std::pair<int, int>(status, true); }

  inline virtual void add_sub_task(int id) {}
  inline virtual int pop_sub_task(int id) { return return_val::none; }
  inline virtual std::vector<int> remove_sub_tasks() { return std::vector<int>(); }
  inline virtual std::vector<int> get_sub_tasks() { return std::vector<int>(); }
...
public:
  ...
  inline std::vector<int> get_sub_tasks() { return sub_tasks; }
...
public:
  ...
  std::map<int, int> perm_desks; //the ratio between username and permission for desks id
#endif
  inline User(std::string username,
       std::string password) : username(username), password(password) {}

  inline std::string get_name() { return username; }
  inline std::string get_password() { return password; }
  int get_permission(int id);
  std::vector<int> get_desks();

  inline void set_name(std::string new_username) { username = new_username; }
  inline void set_password(std::string new_password) { password = new_password; }
  inline void set_perm(int id, int flag) { perm_desks[id] = flag; }
  int pop_perm(int id);
...
public:
  ...
  inline int create_simple_task(int id) { return create_task<Simple_task>(id); }
  inline int create_complex_task(int id) { return create_task<Complex_task>(id); }
  inline int create_desk(int id) { return create_task<Desk>(id); }
...
//methods of classes
//User
inline std::vector<int> User::get_desks()
{
  std::vector<int> res;
...
//logic
inline logic &logic::get_instance()
{
  static logic l;
  return l;
}

inline std::shared_ptr<Simple_task> logic::get_task(int id)
{
...
inline std::vector<int> logic::load_desks(User user)
{
  ptree pt;
...
inline std::pair<int, int> logic::get_status(int id)
{
  auto elem = logic::loaded_users_desks.find(id);
  if (elem != logic::loaded_users_desks.end())
  {
...
template <typename T>
inline int logic::create_task(int id)
{
...
inline int logic::remove_task(int id)
{
...
//Desk
inline int Desk::to_json(ptree &pt)
{
...
//Complex_task
inline void Complex_task::add_sub_task(int id)
{
  sub_tasks.push_back(id);
}

inline std::vector<int> Complex_task::remove_sub_tasks()
{
...
inline int Complex_task::pop_sub_task(int id)
{
...
inline std::pair<int, int> Complex_task::get_status()
{
...
inline int Complex_task::to_json(ptree &pt)
{
...
inline int Simple_task::to_json(ptree &pt)
{
